...
export default function SwingPage() {
  ...
  useEffect(() => {
    ...
    async function loadSwingData() {
      try {
        ...
        comparisons.forEach((row: SwingRow) => {
  ...
  const changedHands = useMemo(() => rows.filter((row) => row.from !== row.to).length, [rows]);
  ...
  return (
    <div className="p-8 max-w-[1920px] mx-auto min-h-screen space-y-6">
      <PageHeader
        ...
        description="Real constituency-level change in winning-margin share between the 2017 and 2022 results."
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <PremiumCard padding="sm" className="text-center"><ArrowRightLeft className="w-5 h-5 text-[var(--accent-primary)] mx-auto mb-2" /><div className="text-2xl font-bold text-[var(--text-primary)]">{loading ? "—" : changedHands}</div><div className="text-xs text-[var(--text-secondary)]">Seats Changed Hands</div></PremiumCard>
        <PremiumCard padding="sm" className="text-center"><TrendingUp className="w-5 h-5 text-[#F97316] mx-auto mb-2" /><div className="text-2xl font-bold text-[var(--text-primary)]">{loading ? "—" : `${maxSwing?.swing.toFixed(1) ?? "0.0"}%`}</div><div className="text-xs text-[var(--text-secondary)]">Max Swing{maxSwing ? ` (${maxSwing.constituency})` : ""}</div></PremiumCard>
        <PremiumCard padding="sm" className="text-center"><TrendingUp className="w-5 h-5 text-emerald-500 mx-auto mb-2" /><div className="text-2xl font-bold text-[var(--text-primary)]">{loading ? "—" : `${averageSwing.toFixed(1)}%`}</div><div className="text-xs text-[var(--text-secondary)]">Avg Swing</div></PremiumCard>
        <PremiumCard padding="sm" className="text-center"><AlertTriangle className="w-5 h-5 text-amber-500 mx-auto mb-2" /><div className="text-2xl font-bold text-[var(--text-primary)]">{loading ? "—" : highSwing}</div><div className="text-xs text-[var(--text-secondary)]">High Swing (&gt;10%)</div></PremiumCard>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <PremiumCard className="p-6 h-[420px] flex flex-col">
          <h2 className="text-lg font-serif font-bold text-[var(--text-primary)] mb-1">Swing Distribution by 2022 Winner</h2>
          <p className="text-xs text-[var(--text-secondary)] mb-4">Change in winning-margin share of votes</p>
          <div className="flex-1 w-full min-h-0"><ResponsiveContainer width="100%" height="100%"><BarChart data={distribution} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}><CartesianGrid strokeDasharray="3 3" stroke="var(--border-subtle)" vertical={false} /><XAxis dataKey="range" stroke="var(--text-secondary)" tick={{ fill: "var(--text-secondary)", fontSize: 12 }} axisLine={false} tickLine={false} /><YAxis stroke="var(--text-secondary)" tick={{ fill: "var(--text-secondary)", fontSize: 12 }} axisLine={false} tickLine={false} /><Tooltip contentStyle={{ backgroundColor: "var(--bg-surface)", border: "1px solid var(--border-subtle)", borderRadius: "8px", color: "var(--text-primary)" }} /><Bar dataKey="toBJP" name="2022 BJP winners" fill="#F97316" stackId="a" /><Bar dataKey="toSP" name="2022 SP winners" fill="#EF4444" stackId="a" /><Bar dataKey="toBSP" name="2022 BSP winners" fill="#2563EB" stackId="a" /><Bar dataKey="toOthers" name="Other 2022 winners" fill="#94A3B8" radius={[4, 4, 0, 0]} stackId="a" /></BarChart></ResponsiveContainer></div>
        </PremiumCard>

        <PremiumCard className="p-6 h-[420px] flex flex-col">
          <h2 className="text-lg font-serif font-bold text-[var(--text-primary)] mb-4">Top 10 Swing Constituencies</h2>
          <div className="flex-1 overflow-y-auto space-y-3 pr-2">
            {loading ? <div className="text-sm text-[var(--text-secondary)]">Loading live election results…</div> : rows.slice(0, 10).map((row, index) => <div key={row.constituency} className="flex items-center gap-4 p-3 rounded-xl bg-[var(--bg-app)] border border-[var(--border-subtle)]"><span className="text-lg font-bold text-[var(--text-tertiary)] w-8 text-center">#{index + 1}</span><div className="flex-1"><div className="text-sm font-semibold text-[var(--text-primary)]">{row.constituency}</div><div className="text-xs text-[var(--text-secondary)]">{row.from} → {row.to}</div></div><span className="text-lg font-bold text-rose-500">{row.swing.toFixed(1)}%</span></div>)}
