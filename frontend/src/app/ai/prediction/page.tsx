...
type VoteEstimate = {
  ...
  margin_basis: "share_implied" | "candidate_contest_regression" | "unavailable"; share_implied_status: string;
...
type Evidence = { snapshot_id?: string; code: string; status: string; freshness: string; items: Source[]; cluster_count?: number;
  ...
  context?: { status: string; limitations: string[]; references: Link[];
    supplemental?: {label: string; value: string; year: number | null; caveat: string; source_url: string}[];
...
function VoteSupportPanel({ row }: {row: Prediction}) {
  ...
  return <details className="text-sm rounded-lg border border-[var(--border-subtle)] p-4">
    ...
    {vote.share_winner_disagreement && <p className="mt-3 text-amber-700 dark:text-amber-300">The mean vote-share leader ({vote.share_leader}) differs from the win-probability leader ({row.predicted_party}). Neither model overrides the other. A separately estimated contest margin is not conditional on either named winner.</p>}
    ...
    <p className="mt-3">Independent top-two candidate contest margin: approximately {vote.contest_margin_votes.toLocaleString()} votes ({vote.contest_margin_pct_points.toFixed(1)} points). The table uses this independently fitted estimate when its historical checks pass and the share-implied margin is not eligible. It can differ from the party-share gap.</p>
...
function EvidencePanel({ row, runId, close }: { row: Prediction; runId: string; close: () => void }) {
  ...
  return <section ref={panel} tabIndex={-1} aria-labelledby="seat-detail-title" className="space-y-5 scroll-mt-24 rounded-xl border border-[var(--border-subtle)] p-5 bg-[var(--bg-card)]">
    ...
    <ResearchPanel endpoint={endpoint} code={row.code} runId={runId} />
    ...
    {evidence && <>
      ...
      <p className="text-sm rounded-lg p-3 bg-amber-500/10 text-amber-700 dark:text-amber-300">Discovery only: these headlines are unreviewed, not polling. OpenAI research, when explicitly run, appears separately above. Location matches and duplicate clustering require review; headline counts alone do not establish an electoral swing.</p>
      ...
      <details className="text-sm"><summary className="cursor-pointer font-medium">Official demographic and economic context</summary>
        <p className="my-3">{demographic ? `${demographic.district_2011} district · Census ${demographic.year} · population ${demographic.population.toLocaleString()}` : "This district is not independently covered by the historical 71-district Census table. Supplemental official context is shown where available."}</p>
        {demographic && <><div className="flex flex-wrap gap-4">{Object.entries(demographic.religion_pct).map(([key, value]) => <span key={key}>{key}: {value.toFixed(2)}%</span>)}</div><div className="flex flex-wrap gap-4 mt-3">{Object.entries(demographic.socioeconomic || {}).map(([key, value]) => <span key={key}>{key.replaceAll("_", " ")}: {value.toFixed(2)}%</span>)}</div></>}
        {evidence.context?.supplemental?.map(item => <article key={item.source_url} className="my-3 border-t pt-3"><p>{item.label}: {item.value} · {item.year ?? "Year not stated"}</p><p className="text-xs mt-1">{item.caveat}</p><a href={item.source_url} target="_blank" rel="noopener noreferrer" className="underline text-xs">District administration source</a></article>)}
        <p className="mt-3 text-amber-700 dark:text-amber-300">{evidence.context?.limitations.join(" ")}</p><div className="flex flex-col gap-2 mt-3">{evidence.context?.references.map(link => <a className="underline" key={link.url} href={link.url} target="_blank" rel="noopener noreferrer">{link.title}</a>)}</div></details>
...
export default function PredictionPage() {
  ...
  const exportRows = (data: Prediction[], filename: string) => downloadCsv(filename, data.map(row => ({
    ...
    margin_basis: row.vote_estimate?.margin_basis,
    caveat: "Review estimates. Win probability is not vote share. Candidate-contest margin estimates contest size independently of the named winner and vote-share gap. Historical error bands are not calibrated future intervals. IPT vote share is pooled, not one candidate.",
  })));
  return <div className="p-4 md:p-8 max-w-[1920px] mx-auto min-h-screen space-y-6">
    ...
    {state?.backtest.vote_support?.beats_or_matches_persistence.share_implied_margin === false && <p className="text-sm p-4 rounded-lg border border-amber-500/40">Share-derived margins did not beat the prior-election baseline. Where eligible, the table shows the separately tested candidate-contest margin instead, labelled as an approximate contest-size estimate. Historical errors remain substantial; inspect Model quality.</p>}
