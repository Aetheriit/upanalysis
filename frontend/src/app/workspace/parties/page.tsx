...
const PARTIES = [
  ...
  { name: "SP", fullName: "Samajwadi Party", seats2017: 47, seats2022: 111, voteShare2017: "21.8%", voteShare2022: "32.1%", color: "#EF4444", allies: "RLD, SBSP, Mahan Dal" },
  { name: "BSP", fullName: "Bahujan Samaj Party", seats2017: 19, seats2022: 1, voteShare2017: "22.2%", voteShare2022: "12.9%", color: "#2563EB", allies: "None" },
  { name: "INC", fullName: "Indian National Congress", seats2017: 7, seats2022: 2, voteShare2017: "6.2%", voteShare2022: "2.3%", color: "#22C55E", allies: "None" },
  { name: "RLD", fullName: "Rashtriya Lok Dal", seats2017: 1, seats2022: 8, voteShare2017: "1.8%", voteShare2022: "2.8%", color: "#EAB308", allies: "SP Alliance" },
  { name: "AIMIM", fullName: "All India MIM", seats2017: 0, seats2022: 0, voteShare2017: "0.2%", voteShare2022: "0.4%", color: "#06B6D4", allies: "None" },
];
...
export default function PartiesPage() {
  ...
  const pieData = PARTIES.filter(p => (is2017 ? p.seats2017 : p.seats2022) > 0).map(p => ({ 
    name: p.name, 
    value: is2017 ? p.seats2017 : p.seats2022, 
    color: p.color 
  }));
  ...
  return (
    <div className="p-8 max-w-[1920px] mx-auto min-h-screen space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        {/* Seat Comparison */}
        <PremiumCard className={`p-6 ${isComparison ? 'lg:col-span-8' : 'lg:col-span-6'} h-[400px] flex flex-col`}>
          <h2 className="text-lg font-serif font-bold text-[var(--text-primary)] mb-4">
            {isComparison ? "Seat Comparison (2017 vs 2022)" : `Seats Won (${activeYear})`}
          </h2>
        ...
        {/* Seat Share Pie */}
        <PremiumCard className={`p-6 ${isComparison ? 'lg:col-span-4' : 'lg:col-span-6'} h-[400px] flex flex-col`}>
          <h2 className="text-lg font-serif font-bold text-[var(--text-primary)] mb-4">{activeYear} Seat Share</h2>
          <div className="flex-1 w-full min-h-0 relative">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie data={pieData} cx="50%" cy="50%" innerRadius={70} outerRadius={110} stroke="none" dataKey="value">
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-[var(--bg-app)]/50 border-b border-[var(--border-subtle)]">
                <th className="px-6 py-3 text-xs font-semibold text-[var(--text-secondary)] uppercase tracking-wider whitespace-nowrap">Party</th>
                <th className="px-6 py-3 text-xs font-semibold text-[var(--text-secondary)] uppercase tracking-wider whitespace-nowrap">Full Name</th>
                {(isComparison || is2017) && <th className="px-6 py-3 text-xs font-semibold text-[var(--text-secondary)] uppercase tracking-wider whitespace-nowrap text-center">Seats '17</th>}
                {(isComparison || is2022) && <th className="px-6 py-3 text-xs font-semibold text-[var(--text-secondary)] uppercase tracking-wider whitespace-nowrap text-center">Seats '22</th>}
                {isComparison && <th className="px-6 py-3 text-xs font-semibold text-[var(--text-secondary)] uppercase tracking-wider whitespace-nowrap text-center">Change</th>}
                <th className="px-6 py-3 text-xs font-semibold text-[var(--text-secondary)] uppercase tracking-wider whitespace-nowrap text-center">Vote Share '{activeYear.slice(2)}</th>
                <th className="px-6 py-3 text-xs font-semibold text-[var(--text-secondary)] uppercase tracking-wider whitespace-nowrap">Alliance Partners</th>
              </tr>
            </thead>
                ...
                <tr key={p.name} className="hover:bg-[var(--bg-app)]/30 transition-colors">
                  ...
                  {isComparison && (
                    <td className="px-6 py-4 text-sm font-medium text-center">
                      <span className={p.seats2022 > p.seats2017 ? "text-emerald-500" : p.seats2022 < p.seats2017 ? "text-rose-500" : "text-[var(--text-tertiary)]"}>
                        {p.seats2022 > p.seats2017 ? "↑" : p.seats2022 < p.seats2017 ? "↓" : "-"} {Math.abs(p.seats2022 - p.seats2017) || ""}
                      </span>
                    </td>
                  )}
                  
                  <td className="px-6 py-4 text-sm font-mono text-[var(--text-primary)] text-center">
                    {is2017 ? p.voteShare2017 : p.voteShare2022}
                  </td>
