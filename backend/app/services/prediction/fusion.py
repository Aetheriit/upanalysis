...
def weights(stat_probabilities: dict[str, float], atmo: dict[str, Any] | None, disruption: bool = False) -> tuple[float, float]:
    ...
    if not math.isfinite(quality) or not atmo.get("probabilities"):
        return 1.0, 0.0
...
def fuse(stat: dict[str, Any], atmo: dict[str, Any] | None) -> dict[str, Any]:
    ...
    agreement = 1 if not atmo_weight or atmo.get("predicted_party") == stat["stat_predicted_party"] else 0
    confidence = ordered[0] * 100
